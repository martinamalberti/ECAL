// per compilare: g++ -Wall -o LocalCorrectionEB `root-config --cflags --glibs` -lboost_regex -lboost_program_options LocalCorrectionEB.cpp
...
int main(int argc, char** argv)
{
  ...
  for(int mod=0;mod<4;mod++){
	...
	TF1 * templateFuncMC = new TF1("templateFuncMC", templateHistoFuncMC[0], eopmin, eopmax, 3, "histoFunc");
	...
	templateFuncMC -> SetParameter(1, 1.0 );
	...
	int fitStatus = -1;
	int nTrials = 0;
	while( (fitStatus != 1) && (nTrials < 3) )
	  {
	    templateFuncMC -> SetParameter(1, 0.99+nTrials*0.01 );
	    fitStatus = h_EoP_MC[i][mod] -> Fit("templateFuncMC", "SMRQL+");
	...
	//	if ( templateFunc->GetParError(1) < 0.0001) g_EoP -> SetPointError(i, 0., 0.003);
	...
	//--- corrected MC  
	templateFuncMC = new TF1("templateFuncMC", templateHistoFuncMC[1], eopmin, eopmax, 3, "histoFunc");
	templateFuncMC -> SetNpx(10000);
	...
	r = h_EoC_MC[i][mod] -> Fit("templateFuncMC", "SMRQL+");
	fitStatus = -1;
	nTrials = 0;
	while( (fitStatus != 1) && (nTrials < 3) )
	  {
	    templateFuncMC -> SetParameter(1, 0.99+nTrials*0.01 );
	    fitStatus = h_EoC_MC[i][mod] -> Fit("templateFuncMC", "SMRQL+");
	    if( fitStatus == 1 ) break;
	    ++nTrials;
	  }
	...
	//************************ DATA **********************************************
	TF1 * templateFuncData = new TF1("templateFuncData", templateHistoFuncData[0], eopmin, eopmax, 3, "histoFunc");
	...
	templateFuncData -> SetParameter(1, 1.02 );
	templateFuncData -> FixParameter(2, 0.);
    

	r =	h_EoP_Data[i][mod] -> Fit("templateFuncData", "MRQLS+");
	fitStatus = -1;
	nTrials = 0;
	while( (fitStatus != 1) && (nTrials < 3) )
	  {
	    templateFuncData -> SetParameter(1, 0.99+nTrials*0.01 );
	    fitStatus = h_EoP_Data[i][mod] -> Fit("templateFuncData", "MRQLS+");
	    if( fitStatus == 1 ) break;
	    ++nTrials;
	  }
...
//    if ( templateFuncData->GetParError(1) < 0.003) g_EoP -> SetPointError(i, 0., 0.003);
	...
	//--- corrected Data 
	templateFuncData = new TF1("templateFuncData", templateHistoFuncData[1], eopmin, eopmax, 3, "histoFunc");
	templateFuncData -> SetNpx(10000);
	...
	r =	h_EoC_Data[i][mod] -> Fit("templateFuncData", "MRQLS+");
	fitStatus = -1;
	nTrials = 0;
	while( (fitStatus != 1) && (nTrials < 3) )
	  {
	    templateFuncData -> SetParameter(1, 0.99+nTrials*0.01 );
	    fitStatus = h_EoC_Data[i][mod] -> Fit("templateFuncData", "MRQLS+");
	    if( fitStatus == 1 ) break;
	    ++nTrials;
	  }
